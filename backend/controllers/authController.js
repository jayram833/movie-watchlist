...
exports.protect = async function (req, res, next) {
    let errorMessage = "";
    if (req.path.includes("delete")) errorMessage = "delete a movie";

    let token;
    if (req.headers.authorization && req.headers.authorization.startsWith("Bearer")) {
        token = req.headers.authorization.split(" ")[1];
    }
    if (!token) return next(new AppError(`You are not logged in! Please log in to ${errorMessage || "get access"}.`, 401));
